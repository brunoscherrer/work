...
If the set $\C^{(1)}$ is empty (which can be checked in polynomial time), it implies that $X^*=\emptyset$ and there is nothing else to do. We therefore assume for what follows that $\C^{(1)}$ is not empty. 
