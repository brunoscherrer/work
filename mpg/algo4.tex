...
\end{align}
For any $c \in \{0,1\}^n$, consider the following operators on $\{0,1\}^n \times \R^n$
\begin{align}
  U_{c,\mu,\nu} (b,v) &= (~ \min(c, P_{\mu,\nu}b),~ T_{\mu,\nu}v ~),\\
  U_{c,\mu} (b,v) & = (~ \min(c,P_{\m  \\
  U_c(b,v) & = \max_{\mu} U_{c,\mu} (b, v).
\end{align}
When solving the $n$-step problem by computing $(U_c)^n(1,v)$, the primary objective of MAX is to only visit nodes $x$ with value $c(x)=1$, and the secondary objective is to maximize the discounted value with terminal value $v$. The objective of MIN is to minimize the 

We shall consider a Policy Improvement step that takes as parameters a threshold $\rho$ and a stationary policy $\mu$, and that returns a non-stationary policy $(\mu_1,\dots,\mu_n)$:
\begin{enumerate}
  \item (Initialization) Set $C=\emptyset$. Compute the value $v$ when MIN plays optimally against $\mu$:
    \begin{align}
      v = T_{\mu} v.
    \end{align}
  \item ($n$-step advantage) For all $x$, set $c(x)=\1_{x \not\in C}$. Solve the $n$-step decision problem and identify $(\mu_1,\dots,\mu_n)$ such that: 
    \begin{align}
      (U_c)^n (1,v) = U_{c,\mu_1}\dots U_{c,\mu_n}(1,v).
    \end{align}
  \item (Identification of converged states) Compute the set
    \begin{align}
      D = \{ x \in X ~;~ [T_{\mu_1}\dots T_{\mu_n}v](x)-v(x) \le (1-\gamma)\rho \},
    \end{align}
    If $D$ is non empty, set $C=C \cup D$. If $C \neq X$, go to step 2. Otherwise stop.
  \end{enumerate}

Since the size of the set $C$ increases at each iteration, it is clear that this procedure halts after at most $n$ iterations.

\begin{lemma}
  The policy $\mu_1,\dots,\mu_n$ is such that:
  \begin{align}
    \forall x \in X \backslash Y, ~~ v_{\mu_1,\dots,\mu_n}(x) &> v_{\mu}(x)+ \frac{\rho}{n} \\
    \forall x \in Y, ~~ v_{\mu',\nu'}(x) &\ge -n(1-\gamma)R + \rho.
  \end{align}
\end{lemma}

\begin{proof}
  Consider the first inequality. When one gets to step 4 of the procedure, we know that $C$ is empty. In other words, for all  $x \in X \backslash Y$, we know that
  \begin{align}
    \delta(x)>(1-\gamma)\rho.
  \end{align}
  Now, for all $x \in X \backslash Y$,
  \begin{align}
    \1_x (v_{\mu',\nu'}-v_{\mu}) & = \1_x (I-\gamma^n P_{\mu_1,\nu'_1(x)} \dots P_{\mu_n,\nu'_n(x)})^{-1} (T'_{\mu_1,\nu'_1(x)} \dots T'_{\mu_n,\nu'_n(x)} v_\mu - v_\mu) \\
    & \ge \1_x (I-\gamma^n P_{\mu_1,\nu'_1(x)} \dots P_{\mu_n,\nu'_n(x)})^{-1} (T'_{\mu_1} \dots T'_{\mu_n} v_\mu - v_\mu) \\
    & = \1_x (I-\gamma^n P_{\mu_1,\nu'_1(x)} \dots P_{\mu_n,\nu'_n(x)})^{-1} ((T')^n v_\mu - v_\mu) \\
    & = \1_x (I-\gamma^n P_{\mu_1,\nu'_1(x)} \dots P_{\mu_n,\nu'_n(x)})^{-1} (\delta) \\
    & > \frac{1-\gamma}{1-\gamma^n}\rho \\
    & \ge \frac{\rho}{n}.
  \end{align}
